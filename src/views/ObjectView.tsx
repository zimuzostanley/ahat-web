    ...
    <div>
      ...
      {elems.length > showCount && (
        <div className="text-xs text-stone-500 dark:text-stone-400 py-2">
          ...
          {" \u2014 "}
          <button className="text-sky-600 dark:text-sky-400 ml-1 hover:underline" onClick={() => setShowCount(Math.min(showCount + 5_000, elems.length))}>show more</button>
          {" "}
          <button className="text-sky-600 dark:text-sky-400 ml-2 hover:underline" onClick={() => setShowCount(elems.length)}>show all</button>
        </div>
      )}
      {total > elems.length && (
