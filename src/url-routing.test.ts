...
// Extract the URL routing logic for testing.
// These functions mirror the ones in App.tsx.

function stateToUrl(view: string, params: Record<string, any>): string {
  switch (view) {
    case "overview": return "/";
    case "rooted": return "/rooted";
    case "object": return `/object?id=0x${(params.id ?? 0).toString(16)}`;
    case "objects": {
      const sp = new URLSearchParams();
      sp.set("id", String(params.siteId ?? 0));
      sp.set("class", params.className ?? "");
      if (params.heap) sp.set("heap", params.heap);
...
interface NavState { view: string; params: Record<string, any> }
