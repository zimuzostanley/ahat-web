...
function SunIcon() {
  return <svg className="ah-theme-toggle__icon" viewBox="0 0 20 20" fill="currentColor">
  ...
  return {
    ...
    view() {
      ...
      const diffControls = readySessions.length > 1 && (
        <div className="ah-diff-controls">
          <span className="ah-diff-controls__label">Diff:</span>
          <select
          ...
          {diffing && diffProgress && (
            <span className="ah-diff-controls__status">{diffProgress.msg}</span>
          )}
        </div>
      );
      ...
      return (
        ...
        >
          {/* Hidden file inputs */}
          <input oncreate={(v: m.VnodeDOM) => { fileEl = v.dom as HTMLInputElement; }} type="file" accept=".hprof" className="ah-hidden" onchange={handleFile} />
          <input oncreate={(v: m.VnodeDOM) => { mapFileEl = v.dom as HTMLInputElement; }} type="file" accept=".txt,.map" className="ah-hidden" onchange={handleMapFile} />

          {/* Header — shown when we have sessions */}
          {sessions.length > 0 && (
            <header className="ah-header">
              <div className="ah-header__bar">
                {/* Logo */}
                <button
                ...
                >
                  <div className="ah-header__logo-icon">A</div>
                  <span className="ah-header__logo-text">ahat<span className="ah-header__logo-suffix">.web</span></span>
                </button>
                ...
                {showTabs ? (
                  ...
                  <Fragment>
                    ...
                    {sessions.map(s => (
                      ...
                      >
                        {s.status === "loading" && <span className="ah-tab__dot ah-tab__dot--loading" />}
                        {s.status === "error" && <span className="ah-tab__dot ah-tab__dot--error" />}
                        <span className="ah-tab__name">{s.name}</span>
                        <button
                  ...
                  <Fragment>
                    {activeSession && (
                      <span className="ah-header__session-name">{activeSession.name}</span>
                    )}
                ...
                <div className="ah-header__actions">
                  ...
                  <div style={{ position: "relative" }}>
                    ...
                    >{"\u22EF"}</button>
                    {menuOpen && (
                      <div className="ah-header__menu">
                        <button className="ah-header__menu-item" onclick={() => { fileEl?.click(); menuOpen = false; }}>
                          Open File
                        </button>
                        {!captureUsed && (
                          <button className="ah-header__menu-item" onclick={() => { captureUsed = true; activeTab = "device"; menuOpen = false; }}>
                            Capture from device
                          </button>
                        )}
                        {activeSession?.status === "ready" && (activeSession.buffer || activeSession.proxy) && (
                          <Fragment>
                            {activeSession.kind === "hprof" && (
                              <button className="ah-header__menu-item" onclick={() => { mapFileEl?.click(); menuOpen = false; }}>
                                Load Mapping
                              </button>
                            )}
                            <button className="ah-header__menu-item" onclick={async () => {
                              menuOpen = false;
                        ...
                        {activeSession && !showTabs && (
                          <button className="ah-header__menu-item--danger" onclick={() => { closeTab(activeSession.id); menuOpen = false; }}>
                            Close
                          </button>
                        )}
                      </div>
              ...
              {showTabs && activeSession?.status === "ready" && activeSession.kind === "hprof" && activeTab !== "device" && (
                <div className="ah-sub-bar">
                  <nav className="ah-nav" style={{ marginLeft: 0 }}>
                    {navItems.map(n => (
                      <button
                        key={n.view}
                        className={`ah-nav-btn${nav.view === n.view ? " ah-nav-btn--active" : ""}`}
          ...
          {isLanding && activeTab === "device" && (
            <div className="ah-landing">
              <div className="ah-landing__theme">
                <ThemeToggle variant="landing" />
              </div>
              <div className="ah-landing__content">
                <div className="ah-landing__header">
                  <div className="ah-landing__logo-row">
                    <div className="ah-landing__logo-icon">A</div>
                    <h1 className="ah-landing__title">
                      ahat<span className="ah-landing__title-suffix">.web</span>
                    </h1>
                  </div>
                  <p className="ah-landing__subtitle">Android Heap Analysis Tool — runs entirely in your browser</p>
                </div>
                <div
                ...
                >
                  <div className="ah-mb-4">
                    <svg className="ah-landing__drop-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
                    </svg>
                  </div>
                  <p className="ah-landing__drop-text">Drop an .hprof file here or click to browse</p>
                  <p className="ah-landing__drop-hint">Supports J2SE HPROF format with Android extensions</p>
                </div>
                <div className="ah-landing__actions">
                  <button
                    className="ah-landing__capture-btn"
                    onclick={() => { captureUsed = true; }}
                  >
                    <span className="ah-landing__capture-inner">
                      <svg className="ah-landing__capture-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                ...
                {error && (
                  <div className="ah-error-banner ah-mt-4">{error}</div>
                )}
              </div>
            </div>
          )}
          ...
          {captureUsed && (
            <div className={activeTab === "device" ? "" : "ah-hidden"}>
              {sessions.length === 0 && (
                <div className="ah-capture-header">
                  <div className="ah-capture-header__inner">
                    <h1 className="ah-capture-header__title">Capture from device</h1>
                    <ThemeToggle variant="landing" />
                  </div>
                </div>
              )}
              <div className={sessions.length > 0 ? "ah-capture-wrap--compact" : "ah-capture-wrap"}>
          ...
          {activeTab !== "device" && activeSession && (
            <Fragment>
              ...
              {activeSession.status === "loading" && (
                <div className="ah-parse-overlay">
                  <div className="ah-parse-card">
                    <h2 className="ah-parse-card__title" title={`Parsing ${activeSession.name}`}>
                      Parsing {activeSession.name}{"\u2026"}
                    </h2>
                    <div className="ah-progress-bar">
                      <div className="ah-progress-bar__fill" style={{ width: activeSession.progress.pct + "%" }} />
                    </div>
                    <p className="ah-parse-card__status" title={activeSession.progress.msg}>{activeSession.progress.msg}</p>
                    <button
              ...
              {activeSession.status === "error" && (
                <div className="ah-parse-overlay">
                  <div className="ah-parse-card--error">
                    <h2 className="ah-parse-card__title--error">Failed to parse {activeSession.name}</h2>
                    <p className="ah-parse-card__body">{activeSession.errorMsg}</p>
                    <button
              ...
              {activeSession.status === "ready" && activeSession.kind === "vmadump" && activeSession.buffer && (
                <main className="ah-main--vmadump">
                  <HexView
